...
def get_event_list(request):
    ...
    if eid !='':
        ...
        try:
            ...
            return JsonResponse({'status':10022,'message':'query resule is empty'})
        else:
...
def add_guest(request):
    ...
    etime=str(event_time).spilt('.')[0]
    timeArray=time.strptime((etime,'%Y-%m-%d %H:%M:%S'))
    e_time=int(time.mktime(timeArray))
    now_time=str(time.time())
    ntime=now_time.spilt('.')[0]
    n_time=int(ntime)

    if n_time>=e_time:
        return JsonResponse({'status':10025,'message':'event has started'})
    try:
        Guest.objects.ceate(realname=realname,phone=int(phone),email=email,sign=0,event_id=int(eid))
...
def user_sign(request):
    ...
    timeArray=time.strptime(etime,'%YY-%m-%d %H:%M:%S')
    e_time=int(time.mktime(timeArray))
